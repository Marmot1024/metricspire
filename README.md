...
> Current status: Phase 2 passed on 2026-09-02. Phase 3 remains in progress. The portable HTTP/OIDC/OCI profile is implemented and locally tested. On 2026-09-04, the company Databricks Apps profile also passed a single-publisher staging path: the packaged Go service ran behind managed ingress, used a dedicated Lakebase schema for its control plane, published immutable v1 and v2 definitions, queried a fixed read-only TPCH fixture with the real user's authorization, switched back to v1, propagated cancellation, and recorded the expected release/query audit events. Results matched the reviewed golden data, and scans found no credential values in the App logs, control tables, or repository. A named non-publisher has since received staging App `CAN_USE` access, but their browser result is still pending. The separate Unity Catalog denial case also remains unverified: Databricks rejects grant inspection or changes on `samples.tpch.orders`, and the current non-publisher inherits broad staging-catalog data privileges, so neither is a valid controlled denial fixture. This is not a production-ready platform release.
...
Databricks Apps changes only the deployment authentication adapter, not the semantic or policy core. Every verified App user receives the configured query-policy role plus `query:execute`; one stable publisher group additionally receives `model:manage`. Interactive SQL uses the request's short-lived token, so Unity Catalog remains authoritative for data access. The successful staging flow and credential scans now verify this behavior for the publisher identity. A named active staging user was verified outside the publisher group, granted only App `CAN_USE`, and is awaiting the browser query/management-denial check. The negative Unity Catalog case needs both a separately reviewed, permission-governed staging fixture and a genuinely least-privilege principal; the public sample and the current broadly privileged user cannot prove that state. Generic OIDC remains the portable self-hosted adapter.
