...
Read-only catalog-grant and group-membership inspection then found that the selected non-publisher inherits broad data privileges on the obvious staging managed catalog. Creating the denial view there would therefore not prove an engine authorization failure. That user remains valid for the ordinary query/management-separation check, but the Unity Catalog denial must use a different genuinely least-privilege principal and an isolated non-business fixture whose inherited and direct grants are reviewed before deployment. The inspection did not create a catalog, schema, view, user, group, or grant.

A second proposed staging reviewer was found to inherit App management and broad data privileges, so that identity is suitable only for UI/query smoke testing and must not be treated as authorization-denial evidence. No additional App grant was needed or added.

The first human screenshot of the deployed page also recorded a deferred pre-demo UI polish item. The current dependency-free page is intentionally an acceptance console, but the Databricks Apps profile misleadingly displays OIDC login/logout controls, retains local `demo`/`commerce` defaults instead of the configured route, exposes raw JSON too prominently, and uses a long single-column governance form. Do not change this during the current identity gate. Before group demonstration, make a small reviewable UI-only pass that distinguishes managed identity from OIDC, derives safe defaults from deployment configuration, prioritizes catalog/query, de-emphasizes publisher-only governance, and improves result readability without adding a frontend framework or duplicating application logic.

The administrator reviewer then completed the bounded browser smoke check through the staging App. Catalog search returned `gross_revenue` from active release `rel_1773762f199c84670d91_r1` with manifest fingerprint `sha256:1773762f199c84670d916867875c834e5066b2fa1c57aee3139878c0f0d9a116`. Query job `job_854016676d60d130a34b42c6da4d2f01` reached `succeeded` with `row_limit=10`, the same release and manifest, physical fingerprint `sha256:8cc1a37b5558b989874696fa405a92f8586fd2caa248e5a2cfa3947be88090e1`, all five reviewed typed rows in the expected order, and `truncated=false`. This closes only the real UI/catalog/query smoke check for that administrator identity. Because the reviewer inherits App `CAN_MANAGE` and broad data privileges, the evidence cannot close either the ordinary-user management-denial gate or the Unity Catalog denial gate; both remain open. The supplied result contains no model-management operation.
