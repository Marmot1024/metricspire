...
- Keep the proven checksum/bootstrap route reviewable and re-run it after material runtime or platform changes. Technical Go execution, dynamic port binding, managed ingress, architecture, and stop/start recovery passed in staging; managed signal delivery remains unverified because the log stream closes on stop. Do not create a production App from this probe or treat it as full-service acceptance.
