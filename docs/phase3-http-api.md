...
The App access-control list currently grants direct `CAN_MANAGE` only to the existing publisher account, plus inherited administrator access. The dedicated `metricspire-publishers-staging` group has exactly that same account as its sole member. Consequently, the current identity can re-prove the successful publisher path but cannot prove either outstanding negative case: there is no non-publisher who can currently enter the App, and this administrator/publisher identity is not a credible Unity Catalog denial subject. This is an identity-fixture gap, not a MetricSpire or Lakebase service failure.
