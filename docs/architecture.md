...
Phase 2 implements one complete backend path: PostgreSQL catalog lifecycle, deterministic semantic planning, and bounded Databricks SQL execution. Phase 3 adds provider-neutral HTTP management/query APIs, generic OIDC authentication, server-resolved trusted inputs, durable query audit, asynchronous jobs, a metric-first public query boundary, a maintainer-oriented governance UI, and explicit runtime composition around the same application services. A full Databricks Apps staging deployment has passed with both a real publisher and a real ordinary query user; the latter could query and was denied draft, publish, and rollback. The redesigned UI and public query route passed staging deployment smoke checks.
...
The App compute was stopped after the initial acceptance to avoid idle cost, then restarted for the usability deployment. The dedicated staging App, Lakebase control data, releases, audit evidence, publisher group, and warehouse binding are intentionally retained for later review or reuse.

## Current exclusions

Full-service self-hosted OIDC deployment and an engine-native denial after MetricSpire authorization remain release-hardening checks, not extra UI completion gates. The UI does not require query users to know a model or write JSON; maintainers use structured fields for the supported basic-metric workflow. The hosted `/mcp` and `/api/v1/mcp` routes are equivalent stateless Streamable HTTP adapters; Databricks Apps clients use the API path to avoid anonymous browser-login redirects. Every request uses the existing authenticator and query permission, creates a principal-scoped backend, and only query submission may propagate the current short-lived execution credential. Job reads and cancellation remain tenant-and-principal scoped. The seven tools expose no management, identity override, raw SQL or internal model route; the stdio-to-HTTP bridge remains only as development compatibility. Shared result cache, message queue, arbitrary SQL, cross-engine joins, and multi-engine routing remain excluded. DuckDB is not a production data engine. ClickHouse, Doris, Trino/Presto, and other adapters must prove conformance independently before being advertised.
