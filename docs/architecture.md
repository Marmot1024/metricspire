...
The resource-free staging probe passed on 2026-09-03. Databricks selected the `amd64` archive and ran Go 1.27.0 on port 8000; authenticated managed-ingress requests returned 200 before and after stop/start. The edge connection was HTTP/2 while the request observed by the Go process was HTTP/1.1, so H2C remains supported but is not required by the observed forwarding path. No PostgreSQL, database, SQL warehouse, secret, production workspace, or business data was contacted. Databricks automatically created the App service principal and reported its two baseline read-only IAM scopes; MetricSpire configured no additional user scopes or resources. Stop/start reached the expected platform states, but the log stream closed before either the late or early shutdown log could be retained, so managed-runtime signal delivery remains unproven. The App was left stopped. PostgreSQL/Lakebase access, SQL warehouse resource configuration, trusted Apps identity, user-authorization token forwarding, and the deployed black-box suite still must pass before this profile is supported.

## Current exclusions

Phase 3 still has no accepted full-service self-hosted enterprise identity-provider deployment or full-service Databricks Apps deployment, MCP transport, shared result cache, message queue, arbitrary SQL, cross-engine joins, or multi-engine routing. The database-free Go runtime probe is deployment evidence, not product acceptance. DuckDB is not a production data engine. ClickHouse, Doris, Trino/Presto, and other adapters must prove conformance independently before being advertised.
