...
Phase 3 has no accepted full-service self-hosted enterprise identity-provider deployment, and the staging profile has not exercised a MetricSpire-authorized user who is denied by Unity Catalog. These are deferred release-hardening checks rather than Phase 3 completion gates. The real ordinary query user's successful query and management denial have already passed. The UI does not require query users to know a model or write JSON; maintainers use structured fields for the supported basic-metric workflow. MCP transport, shared result cache, message queue, arbitrary SQL, cross-engine joins, and multi-engine routing remain excluded. DuckDB is not a production data engine. ClickHouse, Doris, Trino/Presto, and other adapters must prove conformance independently before being advertised.
