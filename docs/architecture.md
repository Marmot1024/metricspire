...
Full-service self-hosted OIDC deployment and an engine-native denial after MetricSpire authorization remain release-hardening checks, not extra UI completion gates. The UI does not require query users to know a model or write JSON; maintainers use structured fields for the supported basic-metric workflow. The MCP stdio bridge calls the same HTTP service with one operator-supplied API identity; it owns no policy, catalog or execution logic and exposes no management or SQL tools. Remote MCP HTTP transport, shared result cache, message queue, arbitrary SQL, cross-engine joins, and multi-engine routing remain excluded. DuckDB is not a production data engine. ClickHouse, Doris, Trino/Presto, and other adapters must prove conformance independently before being advertised.
