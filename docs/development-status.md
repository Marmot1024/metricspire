...
| MCP | Hosted seven-tool contract and real staging queries tested. The formal `metricspire` public-client OAuth connection completed browser login, persisted under the final server name, restored in a fresh Codex process, and ran a bounded Databricks SQL query. Staging also publishes protected-resource metadata for the API path and rejects anonymous initialization before application routing. | Post-expiry refresh, second-user isolation, Claude Code native OAuth acceptance, and a measured first-login/re-authentication UX. Codex recognizes `authentication required`, but automatic browser launch and current-thread tool refresh are client behavior and are not yet accepted. |
